...
namespace GestaoPatrimonios.Applications.Services
{
    public class SolicitacaoTransferenciaService
    {
        ...
        public void Responder(Guid transferenciaId, Guid usuarioId, ResponderSolicitacaoTransferenciaDto dto)
        {
            ...
            StatusTransferencia statusPendente = _repository.BuscarStatusTransferenciaPorNome("Pendente");
            ...
            if (dto.Aprovado)
            {
                ...
                TipoAlteracao tipoAlteracao = _repository.BuscarTipoAlteracaoPorNome("Transferêcia");

                if (tipoAlteracao == null)
                {
                    throw new DomainException("Tipo altereção 'Transferência' não encontrado");
                }
